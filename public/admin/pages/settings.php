...
if (isset($_POST['check_updates'])) {
    try {
        ...
        // Assuming you store the current version in a config file or database
        $currentVersion = $configManager->getSetting('APP_VERSION');

        if (version_compare($latestVersion, $currentVersion, '>')) {
            // Pull the latest changes from GitHub
            shell_exec('git pull origin main');
            $configManager->updateConfig(['APP_VERSION' => $latestVersion]);
            
            // Restore settings after update
            $configManager->updateConfig($currentSettings);
            
            $_SESSION['success'] = "Updated to version $latestVersion successfully";
        } else {
            $_SESSION['success'] = "You are already using the latest version ($currentVersion)";
        }
    } catch (Exception $e) {
